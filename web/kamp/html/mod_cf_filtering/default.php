...
$document->addScript(JURI::root().'templates/'.$template.'/js/general.js');
...
<form method="get" action="<?php echo JRoute::_('index.php?option=com_customfilters&view=products&Itemid='.$Itemid)?>" class="cf_form<?php echo $moduleclass_sfx;?>" id="cf_form_<?php echo $module->id?>">
	...
	foreach($filters_html_array as $key=>$flt_html){
		$categoryclass = "";
		if(strpos($key, 'virtuemart_category')===false){
			$categoryclass = "";
		}
		else {
			$categoryclass = "cf_flt_category ";
		}
	?>
	<div class="cf_flt_wrapper <?php echo $categoryclass; ?>cf_flt_wrapper_id_<?php echo $module->id?> cf_flt_wrapper_<?php echo $direction ?> " id="cf_flt_wrapper_<?php echo $key?>">
