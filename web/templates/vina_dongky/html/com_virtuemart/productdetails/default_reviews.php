...
if ($this->allowRating || $this->allowReview || $this->showRating || $this->showReview) {
	...
	for( $num = 0; $num<=$maxrating; $num++ ) {
		$stars[] = '
				<span title="'.(vmText::_( "COM_VIRTUEMART_RATING_TITLE" ).$num.'/'.$maxrating).'" class="vmicon ratingbox" style="display:inline-block;width:'. 14*$maxrating.'px;">
	...
	if ($review_editable) {
		...
		<form method="post"
			...
			<?php if($this->allowRating and $review_editable) { ?>
				...
				<h4><i style="display: inline-block; margin-right: 5px;" class="fa fa-edit"></i>
...
";
	...
	<?php
		...
		echo '<strong class="customer_name">'.vmText::_( 'COM_VIRTUEMART_DEAR' ).$this->user->name.',</strong><br />';
		echo '<i class="customer_message">'.vmText::_( 'COM_VIRTUEMART_REVIEW_ALREADYDONE' ).'</i>';
	}
}
			...
			foreach ($this->rating_reviews as $review) {				
				...
				if (!empty($this->rating_reviews) && $review->published) {
					...
					<div class="<?php echo $color ?>">						
						<p class="customer_name bold"><?php echo $review->customer." - "; ?></p>
						<span class="date"><?php echo JHtml::date ($review->created_on, vmText::_ ('DATE_FORMAT_LC')); ?></span>
						<p class="vote"><?php echo $stars[(int)$review->review_rating] ?></p>						
