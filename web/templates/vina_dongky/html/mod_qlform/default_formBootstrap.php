...
<form method="post" action="<?php echo $action; ?>" id="mod_qlform_<?php echo $module->id;?>" class="form-horizontal form-validate" <?php if (1==$params->get('fileupload_enabled')) echo ' enctype="multipart/form-data" ';?>>
            ...
            if ($field->hidden): echo $field->input;
                ...
                <div class="control-group <?php echo $field->id;?>">
                    ...
                    <div class="controls <?php echo $field->id;?>">
                        <?php 
						$placeholder = $field->label ? ' placeholder="' . trim(strip_tags($field->label)) . '"' : '';
						echo str_replace(array('input', 'textarea'),array('input' .$placeholder, 'textarea' .$placeholder) ,$field->input);?>
                    </div>
                </div>
                <?php endif;
        endforeach;
