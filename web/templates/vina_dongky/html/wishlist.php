...
if($product_orderable || $notorderable_enabled) {
	...
	if ($tmpl_favbtn_image) $form_deletefavorite .= '<i class="simple-trash-o"></i>';
	...
	if ($result > 0 ){
			...
			else
			{
				...
				{
					$addtofavorites .= '<a class="jutooltip" title="'.JText::_('VM_LANG_ALL_WISHLIST').'" href="'.$url_favlist.'">';
					$addtofavorites .= '<div class="addtofav_aws_icon_active">';
					if ($tmpl_favlist_style == "button") $addtofavorites .= JText::_('VM_LANG_ALL_WISHLIST');
					else $addtofavorites .= '<i class="simple-'.$tmpl_favlist_style.'"></i>';
					$addtofavorites .= '<span>'.JText::_('VM_LANG_ALL_WISHLIST').'</span></div></a>';
				}
			}
	}
	if ($result == 0){
		if($product_id == $favorite_id && $mode == "fav_add") {
			...
			if ($guest_enabled || !$user->guest){
					...
					$addtofavorites .= ' value="" name="addtofavorites" title="'.JText::_('VM_LANG_ADD_TO_WISHLIST').'" >';
					if ($tmpl_favlist_style == "button") 
					{
						if ($tmpl_favbtn_image) $addtofavorites .= '<i class="simple-star"></i>';
						$addtofavorites .= JText::_('VM_ADD_TO_FAVORITES');
					}
					else $addtofavorites .= '<i class="simple-'.$tmpl_favlist_style.'"></i>';
					$addtofavorites .= '<span>'.JText::_('VM_LANG_ADD_TO_WISHLIST').'</span></button>
					...
					$addtofavorites .= '<a class="jutooltip" title="'.JText::_('VM_LANG_ADD_TO_WISHLIST').'" href="'.$finalUrl.'" alt="Login" title="Login">';
					...
					$addtofavorites .= '<span>'.JText::_('VM_LANG_ADD_TO_WISHLIST').'</span></button></a>';
			}
		}
	}
